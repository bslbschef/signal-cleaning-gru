...
# 数据集类：加载txt文件并进行FFT处理（沿时间轴，即第0维）
class WindSpeedDataset(Dataset):
    def __init__(self, sample_dir, label_dir, transform_fft=True):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform_fft:
            # 对每个通道做FFT（默认沿着第0维，即时间维度）
            sample_fft = torch.fft.fft(sample, dim=0)  # 得到 shape: (n, 3) 的复数张量
            label_fft  = torch.fft.fft(label, dim=0)
            # 将复数张量分解成实部和虚部，并在特征维度上拼接，变为 (n, 6)
...
# 1. MLP模型：将(n,6)数据展平后进行全连接处理，再reshape回原形
...
def main():
    ...
    # 模型相关参数
    parser.add_argument('--seq_len', type=int, default=100, help='样本序列长度（仅对MLP模型有效，需展平数据）')
    ...
    # 创建数据集与数据加载器
    dataset = WindSpeedDataset(args.sample_dir, args.label_dir, transform_fft=True)
...
# python your_script.py --model transformer --epochs 20 --batch_size 8 --sample_dir data/samples --label_dir data/labels
